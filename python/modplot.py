...
def axes(ratio_plot=True, figsize=(4, 4), gridspec_update=None,
    ...
    gridspec_kw = {'height_ratios': (
        3.5, 1) if ratio_plot else (1,), 'hspace': 0.0}
    ...
    fig, axes = plt.subplots(
        nsubplots,  gridspec_kw=gridspec_kw, figsize=figsize)
    if nsubplots == 1:
        axes = [axes]
    ...
    if ratio_plot:
        axes[1].set_ylabel(ylabel_ratio, fontsize=14)
    ...
    if xtick_step is not None:
        xticks_locs = [round(xlim[0] + i*xtick_step, 4)
                       for i in range(1+math.floor((xlim[1]-xlim[0])/xtick_step))]
    ...
    if ytick_step is not None:
        yticks_locs = [round(ylim[0] + i*ytick_step, 4)
                       for i in range(1+math.floor((ylim[1]-ylim[0])/ytick_step))]
...
def style(func, **kwargs):
    if func == 'errorbar':
        defaults = {'fmt': 'o', 'lw': 1.5, 'capsize': 1.5,
                    'capthick': 1, 'markersize': 1.5}
    elif func == 'plot':
        defaults = {'lw': 1.5, 'ls': '-'}
    else:
...
def cms_style(*args, **kwargs):
    cms_default = {'func': 'errorbar',
                   'label': 'CMS 2011 Open Data', 'color': 'black'}
...
def sim_style(*args, **kwargs):
    sim_default = {'func': 'errorbar',
                   'label': 'CMS 2011 Simulation', 'color': 'orange'}
...
def gen_style(*args, **kwargs):
    gen_default = {'func': 'plot', 'ls': '--',
                   'label': r'\textsc{Pythia 6} Generation', 'color': 'blue'}
...
def legend(ax=None, order=None, **kwargs):
    legend_opts = {'handlelength': 2.0, 'loc': 'upper right',
                   'frameon': False, 'numpoints': 2}
...
def calc_hist(vals, bins=10, weights=None, density=True):
    ...
    errs = np.asarray([np.linalg.norm(weights[digits == i])
